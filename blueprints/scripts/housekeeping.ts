...
const BLUEPRINTS_DIR = path.resolve(ROOT_DIR, 'blueprints');
const DB_PATH = path.resolve(BLUEPRINTS_DIR, 'agents.db');
...
function runCheck(check: EvaluationCommand): EvaluationResult {
...
function evaluateFitness(): FitnessResult {
...
function readCurrentAgentCode(): string {
  let agentFile = '';

  try {
    agentFile = __filename;
  } catch {
    agentFile = process.argv[1];
  }

  return fs.readFileSync(agentFile, 'utf-8');
}

function runDgmHLoop(iterations = 3) {
